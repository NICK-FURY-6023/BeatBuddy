...
function neb(embed, player, client) {
    let icon = player.queue.current.identifier ? `https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg` : "https://cdn.discordapp.com/attachments/979241850957357138/1022853504026550432/maxresdefault.jpg";
...
async function trackStartEventHandler(msgId, channel, player, track, client) {
    try {
        ...
        let icon = player.queue.current.identifier ? `https://img.youtube.com/vi/${player.queue.current.identifier}/maxresdefault.jpg` : "https://cdn.discordapp.com/attachments/979241850957357138/1022853504026550432/maxresdefault.jpg";
        ...
        if (!message) {
            ...
            let embed2 = new MessageEmbed().setColor(message.client.embedColor).setDescription(`${emojiplay} **Started Playing** - [${track.title}](${track.uri}) - \`[${convertTime(track.duration)}]\``).setImage(icon).setFooter("`Requested by ${player.queue.current.requester.username}`");
