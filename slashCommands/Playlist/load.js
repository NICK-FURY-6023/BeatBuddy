...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (!data) {
            return interaction.editReply({ embeds: [new MessageEmbed() .setColor("#FF00FF").setDescription(`Playlist not found. Please enter the correct playlist name\n\nDo /list To see your Playlist`)] })
        }
        if (!player) return;

        let count = 0;
        const m = await interaction.editReply({ embeds: [new MessageEmbed() .setColor("#FF00FF").setDescription(`Adding ${data.Playlist.length} track(s) from your playlist **${Name}** to the queue.`)] })
