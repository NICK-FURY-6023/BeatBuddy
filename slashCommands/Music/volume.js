...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!volume || volume < 0 || volume > 100) return await interaction.editReply({
      embeds: [new MessageEmbed().setColor("#000000").setDescription(`Usage: ${client.prefix}volume <Number of volume between 0 - 100>`)]
    }).catch(() => { });

    player.setVolume(volume);
    if (volume > player.volume) return await interaction.editReply({
      embeds: [new MessageEmbed().setColor("#000000").setDescription(`${emojivolume} Volume set to: **${volume}%**`)]
    }).catch(() => { });
