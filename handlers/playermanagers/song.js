...
async function song(client, message, args, type, interaction, extras) {
 ...
 .setColor("RANDOM")
  ...
  async function song_() {
    ...
    if (player.state !== "CONNECTED") {
     ...
     .setDescription(`:thumbsup: **Queued :notes: [${res.tracks[0].title}](${res.tracks[0].uri})**`)
       ...
       if(interaction)
      return interaction.reply({ephemeral: true, embeds: [playembed]})
  ...
  async function playlist_() {
    ...
    var playlistembed = new MessageEmbed()
      ...
      if(interaction) interaction.reply({ephemeral: true, embeds: [playlistembed]})
