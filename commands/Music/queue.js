    ...
    sameVoiceChannel: false,
     ...
     let m = ")";
      ...
      if(!player.queue.current){
        return message.channel.send({embeds: [new MessageEmbed().setColor(`#000000`).setDescription(`There Is Nothing Playing.`)]})
      }
     ...
     if (pages.length <= 1) {
      return await message.reply(pages[page] ? `\`\`\`nim\n    ⬐ current track\n${getspaces(player.queue.current.title)}   ${convertTime(player.queue.current.duration - player.position)} left\n    ⬑ current track\n\n${pages[page]}\n\n   This is the end of the queue!\`\`\`` : `\`\`\`nim\n    ⬐ current track\n${getspaces(player.queue.current.title)}   ${convertTime(player.queue.current.duration - player.position)} left\n    ⬑ current track\n\n   This is the end of the queue!\`\`\``)
       ...
       let previousbut = new MessageButton().setCustomId("queueprev").setEmoji("983306316409892914").setStyle("SUCCESS");

            let nextbut = new MessageButton().setCustomId("queuenext").setEmoji("983306397548691456").setStyle("SUCCESS");

            let stopbut = new MessageButton().setCustomId("queuestop").setEmoji("1016689194984280126").setStyle("DANGER");
