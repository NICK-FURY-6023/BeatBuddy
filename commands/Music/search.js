...
module.exports = {
   ...
   execute: async (message, args, client) => { 
     ...
     player = message.client.manager.create({
      ...
      if (Searched.loadType === "PLAYLIST_LOADED") {
         ...
         const thing = new MessageEmbed()
             ...
             .setDescription(`<:queue:978850428223635496> **Added Playlist to queue**\n**${Searched.tracks.length}** Tracks From **${Searched.playlistInfo.name}**`)
      ...
      if (search.startsWith("https://") || search.startsWith("http://")){
        if(res.loadType === "LOAD_FAILED" || res.loadType === "NO_MATCHES"){
          ...
          var track = res.tracks[0];
                ...
                if (!player.playing && !player.paused && !player.queue.size) { 
                    ...
                    var thing = new MessageEmbed()
                        ...
                        .setDescription(`<:queue:978850428223635496> [${track.title.substring(0, 63)}](${track.uri})\n\n**Requester: **<@${track.requester.id}> | **Duration: **\`❯ ${convertTime(track.duration)}\``)
                    return message.channel.send({embeds: [thing]})
                        }
        }
          ...
          var thing = new MessageEmbed()
                    ...
                    .setDescription(`<:queue:978850428223635496> ${res.tracks.length} Songs **${res.playlist.name}**\n\n**Requester: **<@${tracks.requester.id}> | **Duration: **\`❯ [${convertTime(res.playlist.duration)}]\``)
                return message.channel.send({embeds: [thing]})
        }
    ...
    async function first_layer() {
      ...
      //Menu Collections
      message.react("<a:search:981173899884642384>")
      ...
      collector.on('end', collected => {
        menumsg.edit({embeds: [new MessageEmbed().setColor('#000000').setDescription('Cancelled the Selection')], components: [ ]})
      });
    }
      }
    }
